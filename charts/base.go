...
func (bc *BaseConfiguration) JSONNnotEscaped() template.HTML {
	obj := bc.json()
...
func (bc *BaseConfiguration) json() map[string]interface{} {
	...
	if bc.BackgroundColor != "" {
		obj["backgrounfColor"] = bc.BackgroundColor
	}
...
// WithSingleAxisOpts设置单个轴。
func WithSingAxisOpts(opt opts.SingleAxis) GlobalOpts {
	return func(bc *BaseConfiguration) {
		bc.SingleAxis = opt
	}
}
...
// WithGeoComponentOpts设置地理组件。
func WithGeoComponenOpts(opt opts.GeoComponent) GlobalOpts {
	return func(bc *BaseConfiguration) {
		bc.GeoComponent = opt
...
// WithParallelComponentOpts设置并行组件。
